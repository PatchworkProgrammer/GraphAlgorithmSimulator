...
function updateEdges(conn, remove) {
    ...
    const fromIdx = parseInt(conn.sourceId.slice(4));
    const toIdx = parseInt(conn.targetId.slice(4));
    ...
    if (!remove) {
        ...
        const weight = getEdgeWeight(fromIdx, toIdx);

        // UPDATING CONSOLE INPUT IF GRAPH CHANGED VIA UI
        
        const input = document.getElementById("console_input").value
        

        if (weighted_bool){
            const new_entry = fromIdx + " " + toIdx + " " + weight
            if (input.indexOf(new_entry) == -1) {
            const first_line = input.split('\n')[0]
            const node_num = parseInt(first_line.split(' ')[0])
            const edge_num = parseInt(first_line.split(' ')[1]) + 1  //edge added
        ...
        const first_line = input.split('\n')[0]
        const node_num = parseInt(first_line.split(' ')[0])
        const edge_num = parseInt(first_line.split(' ')[1]) - 1  //edge removed
...
function updateGraphInfo_Dijkstra() {
    ...
    for (var i = 0; i < graph_states[current_state].length; i++) {
        ...
        const nodeIdx = graph_states[current_state][i][0];
        const node_id = 'node' + nodeIdx;


        stillInQ[graph_states[current_state][i][0]] = true;

        const parentIdx = graph_states[current_state][i][1];
    ...
    for (var i = 0; i < stillInQ.length; i++) {
        ...
        const node_weight = node.children[0].children[0].children[2].innerHTML;
        const nodeIdx = i;
        const parentIdx = parseInt(node.children[0].children[0].children[1].innerHTML.slice(2));
        ...
        if (stillInQ[i]) {
            ...
            if (node_weight.localeCompare('W:inf') != 0) {
                ...
                //SET EDGE COLOR
                if (i != 0) {
                    const conn = findConnectorByEndpoints(parentIdx, nodeIdx);
                
                    conn.setType("considered");

                    const weight = getEdgeWeight(parentIdx, nodeIdx);
                    conn.addOverlay({
                        type: "Label",
                        options: { label: weight, location: 0.5, cssClass: 'custom_overlay' }
                    });
...
jsPlumbBrowserUI.ready(function () {
    ...
    instance.bind("beforeDrop", function (info, originalEvent) {
        ...
        const fromIdx = parseInt(info.connection.sourceId.slice(4));
        const toIdx = parseInt(info.connection.targetId.slice(4));
    ...
    instance.on(addButton, "click", function (e) {
        ...
        const input = document.getElementById("console_input").value
        const first_line = input.split('\n')[0]
        const node_num = parseInt(first_line.split(' ')[0]) + 1
        const edge_num = parseInt(first_line.split(' ')[1])
    ...
    instance.on(loadGraphBtn, "click", function (e) {
        ...
        const node_count_of_graph = parseInt(lines[0].split(' ')[0]);
        const edge_count = lines[0].split(' ')[1];
        ...
        for (var j = 1; j < lines.length; j++) {
            ...
            const fromIdx = parseInt(lines[j].split(' ')[0]);
            const toIdx = parseInt(lines[j].split(' ')[1]);
            const weight = parseFloat(lines[j].split(' ')[2]);
